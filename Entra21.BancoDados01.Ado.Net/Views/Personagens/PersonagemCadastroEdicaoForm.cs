...
namespace Entra21.BancoDados01.Ado.Net.Views.Personagens
{
    public partial class PersonagemCadastroEdicaoForm : Form
    {
        ...
        public PersonagemCadastroEdicaoForm(Personagem personagem) : this()
        {
            ...
            //Percorrer cada um do itens do tipo do personagens, para seleciopnar o que o usuário teinha cadastrado anteriormente
            for (int i = 0; i < comboBoxTipoPersonagem.Items.Count; i++)
            {
                var tipoPersonagensPercorrido = comboBoxTipoPersonagem.Items[i] as TipoPersonagem;

                if (tipoPersonagensPercorrido.Id == personagem.Id)
                {
                    comboBoxTipoPersonagem.SelectedItem = tipoPersonagensPercorrido;
                    break;
                }
            }

            //Percorrer cada um dos item da editora, para selecionar o que usuário tinha cadastrado
        ...
        {
            //Buscar todos as editoras do banco de dados para persistir o usuario escolher
